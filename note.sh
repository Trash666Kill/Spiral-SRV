...
DNS=''
# Checks if the DNS variable is empty
if [ -z "$DNS" ]; then
    printf "\033[31m*\033[0m ERROR: THE 'DNS' VARIABLE IS EMPTY. PLEASE SET A VALUE.\n"
    exit 1
fi
BASE="SpiralCT"
BASE_CT_FILES=(
...
newct() {
    ...
    if [ $? -eq 0 ]; then
        ...
        local IP_ADDRESS=$(lxc-attach --name "${DNS}" -- /etc/dnsmasq.d/config/grepip.sh)
        ...
        printf "\033[32m*\033[0m IP ADDRESS FIXED.\n"
            ...
            lxc-attach --name "${DNS}" -- sh -c "kill -1 \$(pidof dnsmasq)"
            lxc-attach --name "${DNS}" -- sh -c "echo '$RESULT' >> /etc/dnsmasq.d/config/reservations"
            lxc-attach --name "${DNS}" -- sh -c "kill -1 \$(pidof dnsmasq)"
            }
